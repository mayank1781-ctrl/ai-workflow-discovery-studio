...
import { gradeCapture } from './graders/capture.mjs';
import { gradeFlow } from './graders/flow.mjs';
import { gradeOutput } from './graders/output.mjs';
...
async function runSchedule({ schedule, globalRules, appClient, startIndex }) {
  ...
  async function worker(workerId) {
    while (cursor < schedule.length) {
      ...
      results.push(result);
      await recordFixture(config, result);
      console.log(`    done ${result.personaId}: turns=${result.turns} time=${round(result.wallMs / 1000, 1)}s rails=${result.grades.rails.pass ? 'pass' : 'fail'} recall=${result.grades.capture.recall}`);
    }
  }
...
async function runAndGrade({ persona, globalRules, appClient, runIndex }) {
  ...
  const grades = {
    ...
    capture: gradeCapture(raw, persona),
    output: gradeOutput(raw),
    flow: await gradeFlow(raw, persona, config)
  };
  return { ...raw, grades };
}
...
async function gradeRawResults(rawResults, personas, globalRules) {
  ...
  for (let index = 0; index < rawResults.length; index++) {
    ...
    const grades = {
      ...
      capture: gradeCapture(noGrades, persona),
      output: gradeOutput(noGrades),
      flow: await gradeFlow(noGrades, persona, { ...config, target: 'replay' })
    };
...
async function finish({ generatedAt, personas, results, calibration = null, modeNote }) {
  ...
  console.log('\nAggregate summary');
  console.log(`[ALL] n=${summary.aggregate.n} rails=${summary.aggregate.railsPassPct}% pass (${summary.aggregate.railViolations} viol) captureRecall=${summary.aggregate.captureRecallAvg ?? '-'} capturePrecision=${summary.aggregate.capturePrecisionAvg ?? '-'} flow=${summary.aggregate.flowQualityAvg ?? '-'} flagged=${summary.aggregate.flowFlagged} output=${summary.aggregate.outputCompletionPct}%`);
  console.log('\nWeak spots');
