...
function summarizeRows(label, rows) {
  ...
  const outputComplete = rows.filter((row) => outputGrade(row)?.complete).length;
  const flowScored = rows.filter((row) => Number.isFinite(flowGrade(row)?.score));
  const flowFlagged = rows.filter((row) => flowGrade(row)?.flagged).length;
  const avg = (values) => {
  ...
  return {
    ...
    captureRecall: avg(rows.map((row) => captureGrade(row)?.captureRecall ?? captureGrade(row)?.recall)),
    capturePrecision: avg(rows.map((row) => captureGrade(row)?.capturePrecision ?? captureGrade(row)?.precision)),
    flowQuality: avg(flowScored.map((row) => flowGrade(row).score)),
    flowFlagged,
...
function buildWeakSpots(results) {
  ...
  for (const row of results) {
    ...
    const capture = captureGrade(row);
    const flow = flowGrade(row);
    const output = outputGrade(row);
    if ((capture?.stage?.intake?.recall ?? 1) < 0.75) add('intake', `${row.personaId}: intake recall ${capture.stage.intake.recall}`);
    if ((capture?.stage?.analysis?.recall ?? 1) < 0.65) add('analysis', `${row.personaId}: analysis recall ${capture.stage.analysis.recall}`);
    if (flow?.flagged) add('flow', `${row.personaId}: ${flow.reason || 'judge flagged'}`);
    if (!output?.complete) add('output', `${row.personaId}: ${output?.reason || 'missing output'}`);
    for (const spot of flow?.weakSpots || []) add(`flow: ${spot}`, row.personaId);
  }
...
<body>
  ...
  <main>
    ...
    <section class="grid">
      ...
      ${metric('captureRecall', scoreText(aggregate.captureRecall))}
      ${metric('Capture precision', scoreText(aggregate.capturePrecision))}
      ${metric('Flow quality', scoreText(aggregate.flowQuality))}
    ...
    <h2>Per Persona</h2>
    <table>
      <thead><tr><th>Persona</th><th>Sims</th><th>Rails</th><th>captureRecall</th><th>Capture Precision</th><th>Flow Quality</th><th>Flagged</th><th>Output</th><th>Median Time</th></tr></thead>
      <tbody>
        ${summary.perPersona.map((row) => `<tr><td>${esc(row.label)}</td><td>${row.n}</td><td>${pctText(row.railsPassPct)} (${row.railViolations} viol)</td><td>${scoreText(row.captureRecall)}</td><td>${scoreText(row.capturePrecision)}</td><td>${scoreText(row.flowQuality)}</td><td>${row.flowFlagged}</td><td>${pctText(row.outputCompletionPct)}</td><td>${row.medianWallSec}s</td></tr>`).join('')}
      </tbody>
    </table>
    ...
    <table>
      ...
      <tbody>
        ${results.map((row) => {
          ...
          <td>R ${scoreText(capture.captureRecall ?? capture.recall)} / P ${scoreText(capture.capturePrecision ?? capture.precision)}</td>
          <td>${flow.skipped ? 'skipped' : `${scoreText(flow.score)}${flow.flagged ? ' flagged' : ''}`}</td>
          <td class="${output.complete ? 'ok' : 'bad'}">${esc(output.reason)}</td>
          <td>${(capture.missed || []).slice(0, 5).map((m) => `<span class="pill">${esc(m.type)}: ${esc(m.value)}</span>`).join(' ')}</td>
        </tr>`;
        }).join('')}
      </tbody>
    </table>
...
function captureGrade(row) {
  return row.grades?.['capture-accuracy'] || row.grades?.capture || null;
}

function flowGrade(row) {
  return row.grades?.['flow-quality'] || row.grades?.flow || null;
}

function outputGrade(row) {
  return row.grades?.['output-completion'] || row.grades?.output || null;
}
