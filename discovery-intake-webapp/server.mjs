...
function extractionInstructions() {
  return [
    ...
    "ANCHOR QUESTION: As soon as the workflow has been named, and before any step-level drilldown, set nextQuestion to exactly one open narrative anchor question, worded verbatim: 'Walk me through this workflow from beginning to end — what triggers it, what happens at each stage, and what does it produce?' This is a single open-ended question and should be the second or third question in the flow, immediately after the workflow is named. Use the end-to-end answer to route every structured question that follows. (This narrative anchor is the only question allowed to name more than two things, because the goal is one free-form end-to-end story rather than a checklist.)",
    "SECOND ANCHOR — PAIN + GOVERNANCE: Immediately after the narrative anchor answer, and still before any step-level drilldown, set nextQuestion to exactly one open question that covers both pain/friction and governance together, worded verbatim: 'Where does this feel most painful or slow for your team — and are there any rules, approval thresholds, or compliance requirements that govern how decisions get made?' This is deliberately ONE open question spanning two zones — pain/friction AND rules, decision logic, approval thresholds, and data sensitivity. Mark it as a single open question; do not split it into two, and never ask about pain and governance separately. Pain/friction signals can NEVER come from a document upload or evidence artifact — they must come from the user's own words in conversation, so never infer pain from a document; ask the person.",
    "THIRD ANCHOR — STEP CONFIRMATION: Immediately after the pain + governance anchor, and BEFORE any step-level drilling begins, set nextQuestion to a step-confirmation question that dynamically lists the steps you have inferred from the narrative anchor (Q1) answer, worded verbatim apart from the bracketed list: 'Based on what you've described, I'm seeing these steps: [AI lists the steps it has inferred from the Q1 answer]. Does that look right — anything missing, merged, or named differently?' You must dynamically construct the actual step list from what the user said in the narrative anchor; do not output the literal bracketed placeholder. If no steps have been inferred yet because no narrative anchor answer has been received, acknowledge that and set nextQuestion to the narrative anchor question (Q1) first before proceeding to this step confirmation.",
    "MAX TWO THINGS PER QUESTION: No nextQuestion may ask about more than two things at once. If you need several details, split them into separate questions of at most two related items each. For example, 'Who performs this step, and what system do they use?' (two things) and 'What triggers this step?' (one thing) are fine; asking about inputs, data types, tools, and outputs in one question is not. The only exception is the open narrative anchor question above.",
    "HARD RULE: Every question you generate must ask about at most 2 things. Never use 'and' to join more than 2 topics in a single question. If you need more information, ask a follow-up question next turn.",
    ...
    "When enriching a step, make the nextQuestion feel conversational rather than like a form, and never ask about more than two things at once. Move through these small clusters one question at a time, in order: the actor and the system/tool they use; then the access/source mode; then the input and how it is handled; then data sensitivity; then the trigger; then the output and handoff; then time taken; then human review or approval; then pain or friction; then exceptions or variations.",
...
function chatInstructions() {
  return [
    ...
    "As soon as the user has named the workflow, and before drilling into step-level detail, ask exactly one open narrative anchor question, worded verbatim: 'Walk me through this workflow from beginning to end — what triggers it, what happens at each stage, and what does it produce?' Ask it as the second or third question, and use the end-to-end answer to route everything that follows.",
    "Immediately after the narrative anchor answer, and still before drilling into step-level detail, ask exactly one open question that covers pain and governance together, worded verbatim: 'Where does this feel most painful or slow for your team — and are there any rules, approval thresholds, or compliance requirements that govern how decisions get made?' This is intentionally a single open question spanning two zones — pain/friction AND rules, decision logic, approval thresholds, and data sensitivity. Keep it as one question; do not split it into two, and never ask about pain and governance separately. Pain/friction can never come from a document upload — it must come from the user's own words in conversation, so ask the person rather than inferring it from any file.",
    "Immediately after the pain and governance question, and before drilling into any step-level detail, ask a step-confirmation question that dynamically lists the steps you inferred from the narrative anchor answer, worded verbatim apart from the bracketed list: 'Based on what you've described, I'm seeing these steps: [AI lists the steps it has inferred from the Q1 answer]. Does that look right — anything missing, merged, or named differently?' Build the actual step list from what the user said; never show the literal bracketed placeholder. If no steps have been inferred yet because the narrative anchor has not been answered, acknowledge that and ask the narrative anchor question first before this step confirmation.",
...
function realtimeInstructions() {
  return [
    ...
    "Use a broad-to-specific discovery style. After the starting topic, establish the workflow name. As soon as it is named, and before any step-level detail, ask exactly one open narrative anchor question, worded verbatim: 'Walk me through this workflow from beginning to end — what triggers it, what happens at each stage, and what does it produce?' Ask it as the second or third question and let the answer route what follows; this open narrative question is the only one allowed to name more than two things. Immediately after the narrative anchor answer, ask exactly one open question that covers pain and governance together, worded verbatim: 'Where does this feel most painful or slow for your team — and are there any rules, approval thresholds, or compliance requirements that govern how decisions get made?' This is intentionally a single open question spanning two zones — pain/friction AND rules, decision logic, approval thresholds, and data sensitivity — so keep it as one question, do not split it into two, and never ask about pain and governance separately; pain/friction can never come from a document upload and must come from the user's own words, so ask the person rather than inferring it. Immediately after that, and before drilling into any step-level detail, confirm the steps by dynamically listing the steps you inferred from the narrative anchor answer, worded verbatim apart from the bracketed list: 'Based on what you've described, I'm seeing these steps: [AI lists the steps it has inferred from the Q1 answer]. Does that look right — anything missing, merged, or named differently?' Build the actual step list from what the user said rather than speaking the literal bracketed placeholder; if no steps have been inferred yet because the narrative anchor has not been answered, acknowledge that and ask the narrative anchor question first before this step confirmation. Then build the component overview through separate small questions of at most two items each — the big phases in rough order from start to finish, then the main inputs and data types, then the tools/systems and access/source mode, then the final outputs — rather than one bundled list. Ask for the overall workflow as a rough numbered step-by-step process if the steps are still unclear. Treat phrases like Step 1, Step one, first step, second step, and numbered list markers as process-step anchors. Once a rough skeleton exists, do not ask permission to move on. Treat it as a working skeleton, tell the user they can correct the steps anytime, and automatically start walking Step 1 or the first incomplete step.",
    "When walking a step, ask about at most two things at a time and gather details through separate questions in this order: the actor and their system/tool; then the access/source mode and the action performed; then the input and its in-process handling; then the output and handoff; then the trigger or dependency; then time taken; then human decisions; then pain; then exceptions or variations; then data sensitivity.",
