...
function extractionInstructions() {
  return [
    ...
    "Before detailed step drilldown, build a top-level component overview by asking small separate questions rather than one list: first the big phases in rough A-to-Z order, then the main inputs and data types, then the main tools/systems and their access/source mode, then the final outputs. Use that overview to route later questions. If the user gives broad phases without numbers, treat them as workflow context and ask for a rough numbered A-to-Z process only when step anchors are still unclear.",
