...
function extractionInstructions(gridSummary = "") {
  // PR 3 — extraction prompt rewrite: 3-layer grid structure + 6 principles.
  // This is the system prompt for /api/extract (processing Discovery answers
  // into grid fields). The narrative instructions were rewritten wholesale; the
  // returned JSON is still validated against extractionSchema, and the
  // document-pivot block is still appended when a grid summary is present.
  const prompt = [
    "You are an AI discovery partner helping map business and finance workflows to identify AI automation opportunities.",
    "",
    "GRID STRUCTURE — extract into exactly these 9 fields across 3 layers:",
    "",
    "LAYER 1 — THE FLOW:",
    "- stepName: short label for this workflow step",
    "- whatHappens: primary description of what occurs",
    "- flowAndDependencies: full chain narrative — what starts it, what blocks it mid-step, what ends it, what it hands to next. Single narrative, not split fields.",
    "- volume: frequency and estimated time per instance combined",
    "- systemsTools: all technology, platforms, data stores touched",
    "",
    "LAYER 2 — PEOPLE & FRICTION:",
    "- whoIsInvolved: roles and teams — who owns, touches, approves. Not names, roles.",
    "- painAndRules: what makes it hard, where errors happen, what logic governs decisions, exceptions, workarounds",
    "",
    "LAYER 3 — DATA & RISK:",
    "- dataFlow: what data moves through this step, what happens to it, how it transforms or routes",
    "- sensitivity: how sensitive is the data. Infer regulatory exposure from this — do not ask about regulation separately",
    "",
    "EXTRACTION PRINCIPLES — follow all 6 always:",
    "",
    "P1 SKETCH FIRST: On any first answer, extract the chain skeleton immediately. Populate partial fields now. Do not wait for complete information. Confidence scores reflect completeness, not presence.",
    "",
    "P2 ASSUME MULTI-HOP: Any workflow described as a single step almost certainly has hidden steps before and after. Always probe: what happens right before this? What happens after it leaves this person?",
    "",
    "P3 SYSTEM SIGNALS: If the user mentions an unfamiliar system name, or uses words like \"internal\", \"proprietary\", \"our platform\", \"our tool\" — automatically set sensitivity to medium and flag dataFlow as needing clarification. Do not ask about sensitivity directly.",
    "",
    "P4 DEFAULT GAPS: Always assume these are missing until explicitly stated: volume/frequency, time per instance, who owns the final decision, what \"update\" or \"process\" or \"send\" actually means technically. These feed the next-best-question priority.",
    "",
    ...
    "",
    "CONVERSATION ARC:",
    "Turn 1: Let the user sketch. Extract everything possible. Do not ask for detail yet.",
    "Turn 2+: Zoom in on the highest-priority missing field using P6. One question per turn.",
    "Never ask: \"can you tell me about X, Y, and Z?\" Always ask about one thing only.",
    "",
    "Return only JSON matching the schema."
  ].join("\n");
  const parts = [prompt];
  if (gridSummary && gridSummary.trim()) parts.push(documentExtractionPivotBlock(gridSummary));
  return parts.join("\n\n");
}
